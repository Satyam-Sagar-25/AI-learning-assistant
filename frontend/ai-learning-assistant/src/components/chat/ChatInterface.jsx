...
const ChatInterface = () => {
    ...
    const renderMessage = (msg,index)=>{
        ...
        return (
            <div key={index} className={`flex items-start gap-3 my-4 ${isUser ? 'justify-end':''}`}>{!isUser && (
  ...
  return (
    <div className="flex flex-col h-[70vh] bg-white/80 backdrop-blur-xl border border-slate-200/60 rounded-2xl shadow-xl shadow-slate-200/50 overflow-hidden">
        ...
        <div className="flex-1 p-6 overflow-y-auto bg-linear-to-br from-slate-50/50 via-white/50 to-slate-50/50">
            ...
            <div ref={messagesEndRef}/>{/*This attaches a React ref (messagesEndRef) to that <div> so you can directly access its DOM node. It’s commonly used as a “scroll anchor,” especially in chat apps. */}
            {loading && (
